...
class ConfigHandler(splunk.admin.MConfigHandler):
    ...
    def handleList(self, confInfo):  # pylint: disable=invalid-name, too-many-branches
        ...
        if int(self.callerArgs.get("private_endpoint_enabled", [0])[0]):
            ...
            s3_endpoint_url = "https://{}.{}.amazonaws.com/".format("s3", region)  # pylint: disable=consider-using-f-string
        # Set proxy for boto3
        ...
        try:
            ...
            raise RestError(BAD_REQ_STATUS_CODE, ex)   # pylint: disable=raise-missing-from
