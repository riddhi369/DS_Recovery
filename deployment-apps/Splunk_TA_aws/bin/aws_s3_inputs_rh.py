...
class GenericS3Handler(AdminExternalHandler):
    def handleCreate(self, confInfo):  # pylint: disable=invalid-name
        ...
        if isd and isd != "default" and tsd:
            try:
                ...
                raise RestError(   # pylint: disable=raise-missing-from
                    ...
                    "Wrong datetime format: {}".format(str(ex).capitalize()),  # pylint: disable=consider-using-f-string
                )
        ...
        super(GenericS3Handler, self).handleCreate(confInfo)  # pylint: disable=super-with-arguments
    ...
    def get(self, field_name):
        ...
        field = self.callerArgs.data.get(field_name)  # pylint: disable=redefined-outer-name
        if field and isinstance(field, (tuple, list)) and field[0]:
            field = field[0]
        else:
            field = None

        if isinstance(field, ("".__class__, u"".__class__)):
            field = field.strip()
