...
class ProxyInfo:  # pylint: disable=too-many-instance-attributes
    ...
    def _parse(self):
        ...
        if len(parts) == 1:
            ...
            logger.error(
                "Invalue proxy string {}".format(  # pylint: disable=consider-using-f-string
                    proxystr
                )
            )
            return
        ...
        if len(parts) == 1:
            ...
            logger.error(
                "Invalue proxy string, wrong url {}".format(  # pylint: disable=consider-using-f-string
                    proxystr
                )
            )
            return
        ...
        if account is not None:
            ...
            if len(parts) == 2:
                ...
                logger.error(
                    "Invalue proxy string, wrong user account {}".format(  # pylint: disable=consider-using-f-string
                        proxystr
                    )
                )
                return
