...
class AWSRawCredentials:
    ...
    def client(self, service_name, region_name, boto_session=None, endpoint_url=None):
        """Returns client."""
        if not boto_session:
            boto_session = boto3
        params = {
        ...
        if not endpoint_url:
            if service_name not in ["cloudwatch", "elb", "elbv2"]:
                params[
                    "endpoint_url"
                ] = "https://{}.{}.amazonaws.com/".format(  # pylint: disable=consider-using-f-string
...
class AWSCredentialsCache:
    ...
    def client(self, service_name, region_name, boto_session=None, endpoint_url=None):
        ...
        return self._credentials.client(
            service_name, region_name, boto_session, endpoint_url
        )
