...
class S3KeyZipReader(S3KeyPackageReader):
    ...
    def __init__(self, config, logger):
        super(S3KeyZipReader, self).__init__(  # pylint: disable=super-with-arguments
            config, logger
        )
        zipfile.ZipExtFile.MIN_READ_SIZE = 8192
        self._package_reader = zipfile.ZipFile(  # pylint: disable=consider-using-with
            self._s3reader
        )

    def __next__(self):
        zip_reader = self._package_reader
        for file_path in zip_reader.filelist:
            with zip_reader.open(file_path) as zip_file:
                while 1:
                    if self._seek_left:
                        self._read_pos += len(self._seek_left)
