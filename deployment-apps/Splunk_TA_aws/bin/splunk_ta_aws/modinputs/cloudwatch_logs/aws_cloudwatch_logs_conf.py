...
class AWSCloudWatchLogsConf:
    ...
    def _expand_tasks(self, stanza, creds):
        ...
        try:
            date_time = datetime.strptime(stanza[aclc.only_after].strip(), date_fmt)
        except ValueError:
            logger.error(
        ...
        stream_matcher = re.compile(stanza.get(aclc.stream_matcher, "").strip() or ".*")
            ...
            tasks.append(
                {
                    ...
                    "private_endpoint_enabled": stanza.get("private_endpoint_enabled"),
                    "sts_private_endpoint_url": stanza.get("sts_private_endpoint_url"),
