...
class S3NoticeParser:
    ...
    def _make(cls, record):
        ...
        # ADDON-46852 handling space character in S3 object name
        key = urlparse.unquote_plus(urlparse.unquote(s3object["key"]))
...
class SQSBasedS3PipelineAdapter:  # pylint: disable=too-many-instance-attributes
    ...
    def _process(  # pylint: disable=inconsistent-return-statements
        self, is_expired, message, session
    ):
        try:
            if is_expired():
    ...
    def _open_temp_file(self):
        try:
            ...
            stf = tempfile.SpooledTemporaryFile(  # pylint: disable=consider-using-with
                max_size=max_size, dir=folder
            )
