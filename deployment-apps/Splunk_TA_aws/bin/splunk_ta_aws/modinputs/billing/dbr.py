...
from splunk_ta_aws.common import pymd5, s3util
...
from splunk_ta_aws.common.aws_credentials import (  # isort: skip # pylint: disable=ungrouped-imports
    AWSAccountError,
    AWSIAMRoleError,
)
...
internal_is_secure = True  # pylint: disable=invalid-name
internal_blacklist = ".+"  # pylint: disable=invalid-name

InvoiceIDColumnName = "InvoiceID"  # pylint: disable=invalid-name
DetailBillingReportFileMonthReg = (  # pylint: disable=invalid-name
    r"aws-billing-detailed-line-items-.*(\d{4}-\d{2})\.csv\.zip$"
)
MonthlyBillingReportFileMonthReg = (  # pylint: disable=invalid-name
    r"(\d{4}-\d{2})\.csv$"
)
billing_prefix = "aws_ta_billing"  # pylint: disable=invalid-name
...
def _do_clean(temp_folder):
    ...
    # semgrep failure even though temp_file is closed after its usage is completed. That is why added false positive
    temp_dir = os.path.dirname(
        temp_file.name  # nosemgrep: python.lang.correctness.tempfile.flush.tempfile-without-flush
    )
    temp_file.close()
    ...
    for temp in all_temps:
        try:
            ...
            logger.exception("Failed to remove temp billing file.", tempfile=temp)
...
def clean_left_overs(temp_fodler):
    ...
    try:
        ...
        logger.exception(
            "Failed to clean up temp billing file.", temp_fodler=temp_fodler
        )
...
class CheckPointer:
    ...
    def last_completed_scan_datetime(self):
        """Returns last completed scan datetime."""
    ...
    def last_completed_scan_datetime(self, value):
        ...
        self._data["last_completed_scan_datetime"] = boto.utils.get_ts(
            value.utctimetuple()
        )
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        stanza_name,
        bucket_name,
        last_completed_scan_datetime_init=None,  # pylint: disable=invalid-name
        max_items=None,
    ):
        """
        ...
        safe_filename_prefix = "".join(
            [
                c if c.isalnum() else "_"
                for c in stanza_name[start_offset: start_offset + 20]  # fmt: skip
            ]
        )
        stanza_hexdigest = pymd5.md5(
            "{}_{}".format(  # pylint: disable=consider-using-f-string
                stanza_name, bucket_name
            ).encode("utf-8")
        ).hexdigest()
        self._filepath = os.path.join(
            checkpoint_dir,
            "cp_{}_{}.json".format(  # pylint: disable=consider-using-f-string
    ...
    def _load(  # pylint: disable=invalid-name
        self, stanza_name, bucket_name, last_completed_scan_datetime_init=None
    ):
        ...
        try:
            ...
            if err.errno == errno.ENOENT:
                ...
                logger.debug("boto.utils.get_ts")
                last_completed_scan_datetime = boto.utils.get_ts(
                    last_completed_scan_datetime.utctimetuple()
                )
                logger.debug("boto.utils.get_ts done")
                ...
                logger.log(logging.ERROR, "%s: %s", type(err).__name__, err)
                raise
    ...
    def save(self, force=False):
        ...
        if (
            not self._data_modified_time
            or not force
            and time.time() - self._data_modified_time < 15
        ):
            logger.debug(
                "checkpointer save canceled: not modified or too close to last save"
            )
            return

        stime = time.time()

        tfp = "{}.tmp".format(self.filepath)  # pylint: disable=consider-using-f-string
        ...
        try:
            ...
            with open(tfp, "w") as tmp_f:  # pylint: disable=unspecified-encoding
                json.dump(self._data, tmp_f, indent=2)
            logger.debug("dump to temp file done")
        except Exception as exc:
            logger.log(logging.ERROR, "%s: %s", type(exc).__name__, exc)
            raise
        else:
            try:
        ...
        logger.log(
            logging.INFO,
            "Checkpointer saved %d items in %f seconds to: %s",
    ...
    def delete_item(self, key):
        ...
        try:
            ...
            logger.log(
                logging.WARNING, "delete checkpointer item failed - item does not exist"
            )
    ...
    def get_item(self, key):
        ...
        try:
            ...
            if self._max_items and len(self._data["items"]) >= self._max_items:
                raise self.MaxTrackedItemsReached(  # pylint: disable=raise-missing-from
                    self._max_items
                )
    ...
    def get_oldest_item(self):
        ...
        @return: CheckpointItem or None
        """
        if not len(self._data["items"]):  # pylint: disable=len-as-condition
            return None

        oldest_item = sorted(six.iteritems(self._data["items"]), key=lambda x: x[1][1])[
            0
        ]
        return self.CheckpointItem(self, oldest_item[0], oldest_item[1])
...
class MyScript(smi.Script):  # pylint: disable=too-many-instance-attributes
    ...
    def get_scheme(self):
        ...
        scheme.add_argument(
            smi.Argument(
                "aws_s3_region",
                title="AWS Region",
                description="Region to use for regional endpoint",
        ...
        scheme.add_argument(
            smi.Argument(
                "monthly_report_type", required_on_create=False, required_on_edit=False
            )
        )
        scheme.add_argument(
            smi.Argument(
                "detail_report_type", required_on_create=False, required_on_edit=False
            )
        )
        scheme.add_argument(
            smi.Argument(
                "report_file_match_reg",
                title="Report File Match Regex",
        ...
        scheme.add_argument(
            smi.Argument(
                ...
                title="Initial Scan Date timestamp",
                required_on_create=False,
                required_on_edit=False,
            )
        )
        ...
        scheme.add_argument(
            smi.Argument(
                ...
                title="Column list used for monthly " "report event's timestamp",
                required_on_create=False,
                required_on_edit=False,
            )
        )
        scheme.add_argument(
            smi.Argument(
                ...
                title="Column list used for detail " "report event's timestamp",
                required_on_create=False,
                required_on_edit=False,
            )
        )
        scheme.add_argument(
            smi.Argument(
                ...
                title="Max file size in bytes for CSV file",
                required_on_create=False,
                required_on_edit=False,
            )
        )
        ...
        scheme.add_argument(
            smi.Argument(
                "header_magic_regex", required_on_create=False, required_on_edit=False
            )
        )
        scheme.add_argument(
            smi.Argument(
        ...
        scheme.add_argument(
            smi.Argument(
                "aws_iam_role", required_on_create=False, required_on_edit=False
            )
        )
        scheme.add_argument(
            smi.Argument(
                "temp_folder", required_on_create=False, required_on_edit=False
            )
        )
    ...
    def _get_initial_scan_datetime(self):
        ...
        if not scan_datetime or scan_datetime.strip() in ("default", "0"):
            ...
            server_uri = "https://{host}:{port}".format(  # pylint: disable=consider-using-f-string
    ...
    def validate_input(self, definition):
        ...
        try:
            ...
            report_file_match_reg = (
                self.input_items.get("report_file_match_reg") or ""
            ).strip()
            if report_file_match_reg != "":
                try:
                    ...
                    raise Exception(  # pylint: disable=raise-missing-from
                        "Report File Match Regex configured is not a valid regular expression."
                    )
            else:
                # verify
                monthly_report_type = (
                    self.input_items.get("monthly_report_type") or "None"
                )
                if monthly_report_type not in [
                    "None",
                    "Monthly report",
                    "Monthly cost allocation report",
                ]:
                    raise Exception(
                        "The monthly report type is not valid. Please use one of option: "
                        "'None', 'Monthly report', 'Monthly cost allocation report'"
                    )
                detail_report_type = (
                    self.input_items.get("detail_report_type") or "None"
                )
                if detail_report_type not in [
                    "None",
                    "Detailed billing report",
                    "Detailed billing report with resources and tags",
                ]:
                    raise Exception(
                        "The detail report type is not valid. Please use one of option: "
                ...
                if monthly_report_type == "None" and detail_report_type == "None":
                    raise Exception(
                        "At least select one report type or configure customized report match regex."
                    )

        except Exception as exc:
            logger.error("get exception while validating input: " + str(exc))
    ...
    def _exit_handler(  # pylint: disable=inconsistent-return-statements
        ...
        logger.log(
            logging.INFO,
            "cancellation received. sign num=" + str(signum) + " frame=" + str(frame),
        )
    ...
    @staticmethod
    def get_last_second_stamp_from_month_str(  # pylint: disable=invalid-name
        file_month,
    ):
        """Returns timestamp."""
        ...
        logger.debug(
            "get last second " + str(time_stamp) + " from month: " + file_month
        )
    ...
    def _valid_report(self, key):
        ...
        match = re.search(regex_str, key.name)
        if not match:
            return False
        report_year_month = match.group(1)

        match = re.search(r"^(\d+-\d+)-\d+T", self._init_scan_time)
        init_year_month = match.group(1)
    ...
    def _stream_detail_billing_item(
        self, key, checkpoint_item, checkpointer
    ):  # pylint: disable=too-many-locals, too-many-branches, too-many-statements
        max_size = int(
            self.input_items.get("max_file_size_csv_zip_in_bytes") or 500000000
        )
        ...
        if grps is not None:
            grps = grps.groups()
            file_month = grps[len(grps) - 1:][0].strip()  # fmt: skip

        if file_month == "":
            logger.error(
                "fail to get month from file name, cancel processing file. file="
                + key.name
            )
            return
        ...
        if file_month == this_month:
            logger.warning(
                "Only detail billing report for non-current month is process. ignore it: "
                + key.name
            )
            return
        ...
        logger.info(
            "prepare default timestamp for this file: " + str(default_event_time_stamp)
        )
        logger.debug("create tempfile.NamedTemporaryFile")
        temp_folder = self.input_items.get("temp_folder", None)
        temp_csv_zip_file = (
            tempfile.NamedTemporaryFile(  # pylint: disable=consider-using-with
                prefix=billing_prefix, dir=temp_folder
            )
        )
        logger.debug("create tempfile.NamedTemporaryFile done")
        # temp_csv_folder_path = os.path.splitext(temp_csv_zip_file.name)[0]
        try:
            # semgrep failure even though temp_csv_zip_file is closed at the end after its usage is completed.
            # That is why added false positive
            logger.debug(
                "key.get_contents_to_file : start to download file from S3 bucket to local. remote="
                + key.name
                + " local="
                + temp_csv_zip_file.name  # nosemgrep: python.lang.correctness.tempfile.flush.tempfile-without-flush
            )
        ...
        with FileHolder() as csv_files_monitor:
            ...
            with FileHolder(temp_csv_zip_file):
                ...
                zip_handle = zipfile.ZipFile(  # pylint: disable=consider-using-with
                    temp_csv_zip_file
                )
                ...
                for sub_file in zip_handle.namelist():
                    ...
                    # Very complex logic and there are high chances of code breaking if modified.
                    # Temp files are added in the dict and used later in the code.
                    logger.info(
                        "extracting file from: "
                        + sub_file
                        + ", to: "
                        + temp_file.name  # nosemgrep: python.lang.correctness.tempfile.flush.tempfile-without-flush  # pylint: disable=line-too-long
                    )
            ...
            for (  # pylint: disable=too-many-nested-blocks
                file_name,
                opened_file,
            ) in csv_file_list_set.items():
                try:
                    logger.info(
                        "start to processing file: "
                        + file_name
                        + ", temp file path: "
                        + opened_file.name
                    )
                    if self._canceled or self._check_orphan():
                        logger.warning("canceled by external...exit loop")
                        raise Exception(
                            "canceled by external. exist parsing current file. "
                        )
                    ...
                    if os.name == "nt":
                        ...
                        try:
                            ...
                            logger.warning(
                                "fail to open the reader, use original file handler. reason:"
                                + str(exc)
                            )
                            reader = csv.reader(
                                opened_file,
                    ...
                    # get header_dict
                    look_up_header_count = int(
                        self.input_items.get("header_look_up_max_lines") or 3
                    )
                    ...
                    try:
                        ...
                        raise Exception(err_msg)  # pylint: disable=raise-missing-from

                    index = 0
                    header_dict = {}
                    for el_k in header:
                        header_dict[el_k] = index
                        index += 1
                    ...
                    ts_cl_lst_str = (
                        self.input_items.get("detail_timestamp_select_column_list")
                        or ""
                    )

                    timestamp_column_index_list = []
                    for column in ts_cl_lst_str.rstrip().split("|"):
                        try:
                            timestamp_column_index_list.append(
                                header_dict[column.strip()]
                            )
                        except KeyError:
                    ...
                    if check_point_name in last_scanned_detail_record_set:
                        last_scanned_record_number = (
                            last_scanned_detail_record_set[check_point_name] or -1
                        )
                    ...
                    for row in reader:
                        if self._canceled or self._check_orphan():
                            logger.warning("canceled by external...exit loop")
                            raise Exception(
                                "canceled by external. exist parsing current row. "
                            )
                        ...
                        if row_id <= last_scanned_record_number:
                            logger.debug(
                                "current row is scanned, skip it. row_id="
                                + str(row_id)
                                + " last scanned id="
                                + str(last_scanned_record_number)
                            )
                            continue
                        ...
                        try:
                            ...
                            if len(row) <= min(timestamp_column_index_list):
                                raise Exception(
                                    "row column count doesn't contains date column, row: "
                                    + str(row)
                                )
                            ...
                            if first_row:
                                ...
                                if invoice_index == -1:
                                    logger.error(
                                        "cannot get invoice ID column, exit processing this file."
                                    )
                                    break
                                ...
                                try:
                                    ...
                                    logger.warning(
                                        "fail to get the invoice id, it's not the final version, skip it, "
                            ...
                            if not time_str:
                                ...
                                logger.info(
                                    "there's no available timestamp for current row, use default one: "
                                    + str(row)
                                )
                            else:
                            ...
                            for i in range(length):
                                if "," in row[i]:
                                    data += '{}="{}", '.format(  # pylint: disable=consider-using-f-string
                                        header[i], row[i]
                                    )
                                else:
                                    data += "{}={}, ".format(  # pylint: disable=consider-using-f-string
                            ...
                            data += (
                                ", InputTransactionID=%s"  # pylint: disable=consider-using-f-string
                            ...
                            event = smi.Event(
                                time=event_time,
                                source="s3://"
                                + self.input_items.get("bucket_name")
                                + "/"
                                + key.name,
                                data=data,
                                unbroken=True,
                            ...
                            logger.error(
                                "there's error to handle this row: "
                                + str(exc)
        ...
        temp_csv_zip_file.close()

    def _stream_monthly_billing_item(
        self, key, checkpoint_item, checkpointer
    ):  # pylint: disable=too-many-locals, too-many-branches, too-many-statements
        max_size = int(self.input_items.get("max_file_size_csv_in_bytes") or 1000000)
        if key.size > max_size:
            raise Exception(
                "this file size exceeds the maximum file, size="
                + str(key.size)
                + " configured size="
                + str(max_size)
            )
        ...
        try:  # pylint: disable=too-many-nested-blocks
            ...
            global_timestamp_field_reg_list = [
                x.strip()
                for x in (
            ...
            global_timestamp_extraction = (
                self.input_items.get("monthly_real_timestamp_extraction") or ""
            )
            for reg in global_timestamp_field_reg_list:
                global_timestamp_extraction = global_timestamp_extraction.replace(
                    "%TIME_FORMAT_REGEX%", reg
                )
            ...
            try:
                if global_timestamp_extraction != "":
                    ...
                    if re_search is not None:
                        ...
                        if grps is not None:
                            grps = grps.groups()
                            global_time_str = grps[len(grps) - 1:][0].strip()  # fmt: skip
                            logger.info(
                                "extract global time string for this report: "
                                + global_time_str
                            )
            except Exception as ex:  # pylint: disable=broad-except
                logger.error(
                    "catch exception while parsing global time str: "
                    + str(ex)
                    + ", extraciton setting: "
                    + global_timestamp_extraction
                )
            ...
            ts_cl_lst_str = (
                self.input_items.get("monthly_timestamp_select_column_list") or ""
            )
            ...
            if not timestamp_column_index_list:
                raise Exception("there's no any timestamp column in header")
            logger.info(
                "get time stamp column index list: " + str(timestamp_column_index_list)
            )
            ...
            for row in reader:
                ...
                md5 = pymd5.md5(
                    str(row).encode("utf-8") + str(key.etag).encode("utf-8")
                ).hexdigest()
                last_scanned_non_reserved_record_set = (  # pylint: disable=invalid-name
                    checkpoint_item.position
                )
                ...
                try:
                    ...
                    if md5 in last_scanned_non_reserved_record_set:
                        logger.debug(
                            "skip current row because it's already scanned, row: "
                            + str(row)
                        )
                        continue
                    ...
                    if event_time == 0:
                        logger.warning(
                            "there's no available timestamp for current event, skip it. row: "
                            + str(row)
                        )
                        continue
        ...
        except Exception as excep:  # noqa: F841 # pylint: disable=unused-variable, broad-except
    ...
    def _get_normalized_name(self, file_name, file_name_prefix=""):
        ...
        stanza_hexdigest = pymd5.md5(
            "{}_{}_{}_{}".format(  # pylint: disable=consider-using-f-string
                stanza_name, bucket_name, file_name_prefix, file_name
            )
        ).hexdigest()
        file_path = os.path.join(
            checkpoint_dir,
            "cp_{}_{}_{}".format(  # pylint: disable=consider-using-f-string
                safe_filename_prefix, stanza_hexdigest, os.path.splitext(file_name)[1]
            ),
        )
    ...
    def convert_timestamp(
        ...
        for time_format in time_format_list:
            try:
                ...
                time_sec_float = calendar.timegm(date_time.timetuple())
                logger.debug(
                    "Convert time '%s' to UTC time '%s'"  # pylint: disable=consider-using-f-string
                    % (
                        date_time,
                        time_sec_float,
                    )
        ...
        logger.warning(
            "fail to convert timestamp, use now instead. time str="
            + time_str
            + " format list="
            + str(time_format_list)
        )
        return time.time()
    ...
    def _stream_events(
        ...
        """helper function"""
        level = get_level("aws_billing", self.service.token, appName=APPNAME)
        set_log_level(level)
        logger.log(
            logging.INFO,
        ...
        try:
            ...
            logger.warning(
                "Fail to set signal, skip this step: %s: %s", type(exc).__name__, exc
            )
            logger.error(traceback.format_exc())

        file_match_list = {
            "Monthly report": r"\d+-aws-billing-csv-.*\d{4}-\d{2}\.csv",
            "Monthly cost allocation report": r"\d+-aws-cost-allocation-.*\d{4}-\d{2}\.csv",
            "Detailed billing report": r"\d+-aws-billing-detailed-line-items-.*\d{4}-\d{2}\.csv\.zip",
            "Detailed billing report with resources and tags": r"\d+-aws-billing-detailed-line-items-with-resources-and-tags-.*\d{4}-\d{2}\.csv\.zip",  # pylint: disable=line-too-long
        }
        ...
        self._cred_service = create_credentials_service(  # pylint: disable=attribute-defined-outside-init
        ...
        try:
            cred = self._cred_service.load(
                ...
                region_name=self.input_items.get("aws_s3_region"),
            )
        except (  # noqa: F841
            botocore.exceptions.ClientError,
            AWSIAMRoleError,
            AWSAccountError,
        ) as err:  # pylint: disable=unused-variable
            msg = (
                "Failed to load AWS credentials for aws_account={},"  # pylint: disable=consider-using-f-string
                "aws_iam_role={}, Error={}"
            ).format(
                self.input_items["aws_account"],
                self.input_items.get("aws_iam_role"),
            ...
            msg = (
                "Unexpected error happened when "  # pylint: disable=consider-using-f-string
        ...
        try:
            logger.info(
                "start to connecting...input name: "
                + str(self.input_name)
                + " input items: "
            ...
            with CheckPointer(
                ...
                self.input_name,
                self.input_items["bucket_name"],
            ) as checkpointer:
                logger.debug("loop aws_common.s3util.get_keys")
                for key in s3util.get_keys(
                    s3_bucket,
                    ...
                    try:
                        ...
                        if not cp_item:
                            logger.info(
                                "cancel processing this file,"
                                " because it's already been scanned completely and no change: "
                                + key.name
                            )
                            continue
                        ...
                        logger.log(
                            logging.INFO,
                            "Starting: bucket: %r key: %r etag: %s retry_count: %d"
                            ...
                            key.name,
                            key.etag,
                            cp_item.attempt_count,
                            key.size,
                            len(start_position),
                            key.storage_class,
                            key.last_modified,
                            datetime.datetime.utcnow()
                            - boto.utils.parse_ts(key.last_modified),
                        )

                        if key.name.endswith(".csv"):
                            logger.info("the file is csv monthly report. ")
                            self._stream_monthly_billing_item(
                                key, cp_item, checkpointer
                            )
                        elif key.name.endswith(".csv.zip"):
                            logger.info("the file is csv.zip detail report. ")
                            self._stream_detail_billing_item(key, cp_item, checkpointer)

                    except CheckPointer.MaxTrackedItemsReached as err:
                        logger.log(logging.ERROR, "{}: {}", type(err).__name__, err)
                        break
                    except Exception as exc:  # pylint: disable=broad-except
                        if isinstance(exc, boto.exception.S3ResponseError):
                            msg = "{} {} - {} - {}".format(  # pylint: disable=consider-using-f-string
                                exc.status,  # pylint: disable=no-member
                                exc.reason,  # pylint: disable=no-member
                                exc.error_code,  # pylint: disable=no-member
                                exc.error_message,  # pylint: disable=no-member
                            )
                        else:
                            msg = exc
                        logger.log(
                            logging.ERROR,
                            "Incomplete: bucket: %r key: %r etag: %s attempt_number: %d"
                            " orig_size: %d stream_position: %d"
                            " Exception: %s: %s",
                            s3_bucket.name,
                            key.name,
                        ...
                        logger.log(
                            level,
                            "%s: bucket: %r key: %r etag: %s attempt_number: %d"
            ...
            msg = "{} {} - {} - {}".format(  # pylint: disable=consider-using-f-string
                err.status, err.reason, err.error_code, err.error_message
            )
            logger.log(logging.FATAL, "Outer catchall: %s: %s", type(err).__name__, msg)
            logger.error(traceback.format_exc())
            raise
        except Exception as exc:
    ...
    def _refresh_credentials(self, s3_bucket):
        # semgrep false positive, because not exposing any credential details
        logger.info(  # nosemgrep: python.lang.security.audit.logging.logger-credential-leak.python-logger-credential-disclosure  # pylint: disable=line-too-long
            'Refresh credentials started, datainput="%s", bucket_name="%s".',
            self.input_name,
            s3_bucket.name,
        )
        cred = self._cred_service.load(
        ...
        # semgrep false positive, because not exposing any credential details
        logger.info(  # nosemgrep: python.lang.security.audit.logging.logger-credential-leak.python-logger-credential-disclosure  # pylint: disable=line-too-long
            'Refresh credentials done, datainput="%s", bucket_name="%s".',
            self.input_name,
            s3_bucket.name,
        )
        return cred
...
class FileHolder:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        for folder_name in self.folder_list:
            try:
                ...
                logger.error(
                    "fail to delete file: folder name="
                    + folder_name
                    + " error="
                    + str(exc)
                    + str(exc_type)
    ...
    def add_folder_monitor(self, folder_name):
        """Returns file monitor."""
        self.folder_list.add(folder_name)

    def file_list(self):  # pylint: disable=method-hidden
        """Returns file list."""
