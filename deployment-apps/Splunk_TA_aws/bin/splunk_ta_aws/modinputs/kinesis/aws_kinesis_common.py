...
class KinesisClient:  # pylint: disable=too-many-instance-attributes
    ...
    def _create_kinesis_client(self, credentials):
        if not self._kinesis_endpoint_url:
            self._kinesis_endpoint_url = "https://{}.{}.amazonaws.com/".format(  # pylint: disable=consider-using-f-string
        ...
        while 1:
            ...
            try:
                response = self._client.get_records(
                    ShardIterator=shard_iter, Limit=10000
                )
                ...
                raise StopIteration()  # pylint: disable=stop-iteration-return, raise-missing-from
            except Exception:
