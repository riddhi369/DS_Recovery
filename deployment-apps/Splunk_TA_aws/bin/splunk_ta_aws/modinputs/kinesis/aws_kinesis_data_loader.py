...
class KinesisDataLoader:
    ...
    def __init__(self, config):
        ...
        self._msg = (
            "from Kinesis stream={}, shard_id={}, "  # pylint: disable=consider-using-f-string
            "datainput={}"
        ).format(
    ...
    def _do_index_data(self):
        ...
        for records in self._client.get_records():
            ...
            if total_indexed >= 1000:
                logger.info(
                    "Collecting %s for 1000 takes=%s seconds",
                    self._msg,
                    time.time() - start,
                )
                start = time.time()
    ...
    def _index_data(self, records):
        ...
        try:
            ...
            logger.error(
                "Failed to index events %s, error=%s", self._msg, traceback.format_exc()
            )
            raise
