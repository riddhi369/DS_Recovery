...
#
"""
File for VPC description of description input
"""
...
def connect_vpc_to_region(config):
    """Returns connection from VPC to region."""
...
def vpc_network_acls(config):
    """Returns VPC network ACL results."""
    keys = ["associations", "default", "id", "network_acl_entries", "tags", "vpc_id"]
    ...
    results = desc.pop_description_results(
        net_acls, keys, {tac.account_id: config[tac.account_id]}, raw_event=True
    )

    for i, result in enumerate(results):
        result["associations"] = desc.pop_description_results(
            result["associations"], asso_keys, {}, pop_region_name=False, raw_event=True
        )

        result["network_acl_entries"] = desc.pop_description_results(
            result["network_acl_entries"],
            entry_keys,
            {},
        ...
        for entry in result["network_acl_entries"]:
            ...
            entry["icmp"] = desc.pop_description_result(
                entry["icmp"], icmp_keys, {}, pop_region_name=False, raw_event=True
            )
