...
#
"""
File for RDD description for description input.
"""
...
def rds_instances(config, **kwargs):
    ...
    keys = [
        ...
        "read_replica_dbinstance_identifiers",
        "status",
        "status_infos",
        "subnet_group",
        "vpc_security_groups",
    ...
    # security_groups, pagination
    # https://github.com/boto/boto/issues/3109

    conn = connect_rds_to_region(config)
    instances = conn.get_all_dbinstances(**kwargs)
    pagination = {"marker": instances.marker}

    region = {"region": config[tac.region], tac.account_id: config[tac.account_id]}
