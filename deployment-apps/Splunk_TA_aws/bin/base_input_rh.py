...
class BaseInputRestHandler(admin.MConfigHandler):
    ...
    def handleList(self, confInfo):
        ...
        logger.info("%s: listing %d inputs in total" % (self.input_name, len(inputs)))  # pylint: disable=consider-using-f-string

        inputs_dict = merge_util.merge_inputs(
            self.input_name, inputs, self.group_fields
        )

        logger.info("%s: merged into %d inputs" % (self.input_name, len(inputs_dict)))  # pylint: disable=consider-using-f-string
    ...
    def handleEdit(self, confInfo):  # pylint: disable=invalid-name  # pylint: disable=unused-argument
        ...
        logger.info("has_group_fields = %s" % str(has_group_fields))  # pylint: disable=consider-using-f-string
    ...
    def handleCreate(self, confInfo):  # pylint: disable=unused-argument
        ...
        if len(old_inputs) > 0:
            raise RestError(400, ' Name "%s" is already in use' % name)  # pylint: disable=consider-using-f-string
    ...
    def handleRemove(self, confInfo):  # pylint: disable=unused-argument
        ...
        logger.info("%s: removing inputs (%s)" % (self.input_name, self.callerArgs.id))  # pylint: disable=consider-using-f-string
        ...
        for index, (name) in enumerate(editing_inputs, 1):
            ...
            try:
                ...
                if is_entity_obj:
                    ...
                    self._collection.update(name, **updated_dict)  # pylint: disable=no-member
    ...
    def _get_inputs_by_name(self, name):
        ...
        logger.info(
            "%s: %d inputs after filtering." % (self.input_name, len(inputs_dict))  # pylint: disable=consider-using-f-string
        )
