    ...
    def handleList(self, conf_info):  # pylint: disable=invalid-name
        ...
        for required in self.valid_params:
            if not self.callerArgs or not self.callerArgs.get(required):
                ...
                raise Exception('Missing "{}"'.format(required))  # pylint: disable=consider-using-f-string
    ...
    def _list_rules(self, conf_info):
        ...
        while 1:
            try:
                ...
                msg = str(e)
                logger.error(msg)
                raise RestError(400, "Failed to describe config rules: " + msg)   # pylint: disable=raise-missing-from
