...
STS_PRIVATE_ENDPOINT_PATTERN = PRIVATE_ENDPOINT_PATTERN.replace("<service_name>", "sts").replace("<prefix>", "")
...
class AccountRestHandler(admin.MConfigHandler):
    ...
    def handleCreate(self, confInfo):  # pylint: disable=invalid-name # pylint: disable=unused-argument
        ...
        try:
            ...
            if self.callerArgs.id.lower() in ("default",):
                raise RestError(
                    400,
                    'Name "%s" for AWS account is not allowed.' % self.callerArgs.id,  # pylint: disable=consider-using-f-string
                )
            accs = self.all()
    ...
    def handleRemove(self, confInfo):  # pylint: disable=invalid-name # pylint: disable=unused-argument
    ...
    def handleEdit(self, confInfo):  # pylint: disable=invalid-name # pylint: disable=unused-argument
    ...
    def get(self, name):
        ...
        if key is None:
            raise Exception('No AWS account named "%s" exists.' % (name))  # pylint: disable=consider-using-f-string
        cm = self._getConfManager()
